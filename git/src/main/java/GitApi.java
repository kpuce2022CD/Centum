...
public interface GitApi {
    ...
    String DEFAULT_PATH = "D:\\gittest\\";
    boolean gitAccess(String personalToken);
    boolean getAllPublicRepo();



    static void downloadDirectory(GHContent content, String path) {
        ...
        if (content.isDirectory()) {
            ...
            LOG.config(content.getName());
            String[] isPy = content.getName().split("\\.");
            if ((isPy[isPy.length-1].equals("py")) || (isPy[isPy.length-1].equals("pyc")) || (isPy[isPy.length-1].equals("pyo"))){
