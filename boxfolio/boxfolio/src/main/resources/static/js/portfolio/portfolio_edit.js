var json_data;
var mode='private'; //공개 설정. 디폴트는 private
var make=[]; //제작중인 단
var make_src=[];
var make_what=[];
var number=0; //단 개수
...
let publicStatusSwitch = document.getElementById('public-status');
let portfolioBox = document.getElementById('portfolio-box');
let portfolioSendForm = document.getElementById('portfolio-send-form');

publicStatusSwitch.addEventListener('change', ev => {
    if (publicStatusSwitch.checked == true) {
        mode = 'public';
    } else {
        mode = 'private';
    }
...
function video_up(){
    ...
    make[number].firstElementChild.firstElementChild.value=number;
    make[number].style.display = "flex";
    make[number].style.alignItems = "center";
    portfolioBox.append(make[number]);
...
function data_in(){
    ...
    portfolioSendForm.appendChild(document.getElementById('public-status'));
    portfolioSendForm.submit();
}